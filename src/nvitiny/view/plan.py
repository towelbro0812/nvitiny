...
def _fit_tier(width: int, height: int, gpu_count: int) -> tuple[str, int]:
    """高度不足時逐級降版面。
    ...
    tier = tier_for(width)
    while True:
        reserve = 1 if tier == L else 0
        if tier == XS or gpu_count * FIXED_LINES[tier] <= height - reserve:
            return tier, reserve
        tier = DOWNGRADE[tier]
...
def plan(
    ...
    *,
    ...
    tier, reserve = _fit_tier(width, height, max(1, gpu_count))
    per_gpu = FIXED_LINES[tier]

    # 降到 XS 仍放不下所有卡，就少顯示幾張並留一行說明
    shown = gpu_count
    hidden = 0
    if gpu_count * per_gpu > height - reserve:
        shown = max(0, (height - reserve - 1) // per_gpu)
        hidden = gpu_count - shown
    needs = list(proc_counts[:shown])

    budget = height - reserve - shown * per_gpu - (1 if hidden else 0)
    ...
    # 版本行是靜態資訊，優先權低於「讓每張卡的 process 清單有個交代」
    show_version = has_version and budget - 1 >= floor
    if show_version:
        budget -= 1
    ...
    draft = LayoutPlan(
        ...
        show_version=show_version, show_host=tier == L,
